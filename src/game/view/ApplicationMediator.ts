	...
	export class ApplicationMediator extends puremvc.Mediator implements puremvc.IMediator{
        ...
        private doMove(direction:number):void
        {
            if(CommonData.isRunning && (egret.getTimer() - this.lastMoveTime)>=150) {
                switch (direction) {
                    case 0:
                        this.sendNotification(game.controller.GameCommand.USER_MOVE, 0);    //上
                        break;
                    case 1:
                        this.sendNotification(game.controller.GameCommand.USER_MOVE, 1);    //右
                        break;
                    case 2:
                        this.sendNotification(game.controller.GameCommand.USER_MOVE, 2);    //下
                        break;
                    case 3:
                        this.sendNotification(game.controller.GameCommand.USER_MOVE, 3);    //左
                        break;
                }
